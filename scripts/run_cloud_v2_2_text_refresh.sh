...
BASE_ROOT="${V2_2_BASE_OUTPUT_ROOT:-artifacts/v2_2_hybrid}"
OUTPUT_ROOT="${V2_2_OUTPUT_ROOT:-artifacts/v2_2_semantic_refresh}"
BASE_PREDICTIONS="${V2_2_BASE_PREDICTIONS:-${BASE_ROOT}/base/valid_predictions.parquet}"
...
if [[ ! -f "${BASE_PREDICTIONS}" ]]; then
  echo "v2.2 base predictions do not exist: ${BASE_PREDICTIONS}"
  echo "Run scripts/run_cloud_v2_2_hybrid.sh first, or set V2_2_BASE_PREDICTIONS."
  exit 2
fi
...
echo "v2.2 semantic-rule validation completed: ${OUTPUT_ROOT}"
