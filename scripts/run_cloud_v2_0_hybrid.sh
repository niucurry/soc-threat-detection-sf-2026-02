...
OUTPUT_ROOT="${V2_0_OUTPUT_ROOT:-artifacts/v2_0_hybrid}"
PROCESSED_DIR="${V2_0_PROCESSED_ROOT:-data/processed}"
...
python -u src/train_npu_tabular.py \
  ...
  --device auto \
  --epochs "${V2_0_EPOCHS:-12}" \
  --batch-size "${V2_0_BATCH_SIZE:-8192}" \
  --learning-rate "${V2_0_LEARNING_RATE:-0.002}" \
  --class-weight-power "${V2_0_CLASS_WEIGHT_POWER:-0.0}" \
  --patience "${V2_0_PATIENCE:-3}" \
  --num-workers "${V2_0_NUM_WORKERS:-4}" \
