...
OUTPUT_ROOT="${V2_2_OUTPUT_ROOT:-artifacts/v2_2_hybrid}"
PROCESSED_DIR="${V2_2_PROCESSED_ROOT:-data/processed}"
...
python -u src/train_npu_tabular.py \
  ...
  --device auto \
  --epochs "${V2_2_EPOCHS:-12}" \
  --batch-size "${V2_2_BATCH_SIZE:-8192}" \
  --learning-rate "${V2_2_LEARNING_RATE:-0.002}" \
  --class-weight-power "${V2_2_CLASS_WEIGHT_POWER:-0.0}" \
  --patience "${V2_2_PATIENCE:-3}" \
  --num-workers "${V2_2_NUM_WORKERS:-4}" \
