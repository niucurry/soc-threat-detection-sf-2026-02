...
MULTIVIEW_CONTENT_VIEWS = ("head", "middle", "tail", "key_value")
DEFAULT_TOKENS_PER_VIEW = 64
DEFAULT_MESSAGE_CHARS_PER_VIEW = 4096
...
class MultiViewContentEncoding:
...
def key_value_symbols(
    ...
    *,
    max_tokens: int,
    chars_per_region: int = DEFAULT_MESSAGE_CHARS_PER_VIEW,
) -> list[str]:
...
def encode_multiview_content(
    ...
    *,
    buckets: int,
    tokens_per_view: int = DEFAULT_TOKENS_PER_VIEW,
    chars_per_view: int = DEFAULT_MESSAGE_CHARS_PER_VIEW,
    ...
    for symbols in symbol_views:
        ...
        ids.extend(encoded)
        counts.append(count)
    return MultiViewContentEncoding(
