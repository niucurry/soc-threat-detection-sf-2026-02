...
from soc_threat.multiview_content_features import (
    DEFAULT_MESSAGE_CHARS_PER_VIEW,
    DEFAULT_TOKENS_PER_VIEW,
    MULTIVIEW_CONTENT_VIEWS,
    MultiViewContentEncoding,
...
def _encoding_table(
    event_ids: list[str],
    encodings: list[MultiViewContentEncoding],
    *,
    tokens_per_view: int,
) -> pa.Table:
    width = len(MULTIVIEW_CONTENT_VIEWS) * tokens_per_view
    return pa.table(
        {
...
def prepare_shards(
    ...
    *,
    ...
    expected_width = len(MULTIVIEW_CONTENT_VIEWS) * tokens_per_view
    processed = 0
    reused_rows = 0
    written_rows = 0
    ...
    for shard_index, batch in enumerate(
        ...
        if processed >= next_progress:
            print(
                json.dumps(
                    {
                        "stage": "prepare_v4_1_multiview_shards",
                        "input": raw_path.name,
                        "rows": processed,
...
def join_content_and_multiview(
    content_path: Path,
    shard_dir: Path,
    output_path: Path,
    *,
    ...
    connection.execute(
        ...
        COPY (
            SELECT
                ...
                c.* EXCLUDE (event_id)
            FROM read_parquet('{sql_path(content_path)}') AS b
    ...
    if audit[0] != expected_rows or audit[0] != audit[1]:
        temporary.unlink(missing_ok=True)
        raise ValueError(
            f"v4.1 join mismatch for {output_path}: expected {expected_rows}, "
            f"got rows={audit[0]}, distinct={audit[1]}"
        )
    os.replace(temporary, output_path)
    return {
        "base": str(content_path),
        "content_shards": str(shard_dir),
        "output": str(output_path),
        "rows": int(audit[0]),
        "reused": False,
        "average_tokens": dict(zip(MULTIVIEW_CONTENT_VIEWS, map(float, audit[2:]))),
        "size_mb": round(output_path.stat().st_size / 1024**2, 2),
        "seconds": round(time.perf_counter() - started, 2),
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare resumable v4.1 head/middle/tail/key-value content views"
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "data" / "processed" / "v4_1",
    )
    parser.add_argument("--batch-size", type=int, default=20_000)
    parser.add_argument("--progress-every", type=int, default=100_000)
    parser.add_argument("--hash-buckets", type=int, default=DEFAULT_HASH_BUCKETS)
    parser.add_argument(
        "--tokens-per-view", type=int, default=DEFAULT_TOKENS_PER_VIEW
    )
    parser.add_argument(
        "--chars-per-view", type=int, default=DEFAULT_MESSAGE_CHARS_PER_VIEW
    )
...
def main() -> None:
    ...
    if args.tokens_per_view < 16 or args.chars_per_view < 256:
        raise ValueError("v4.1 view limits are too small")
    raw_train = args.data_dir / "train.parquet"
    raw_valid = args.data_dir / "valid_input.parquet"
    content_train = args.content_feature_dir / "content_train.parquet"
    content_valid = args.content_feature_dir / "content_valid.parquet"
    for path in (raw_train, raw_valid, content_train, content_valid):
        if not path.is_file():
            raise FileNotFoundError(path)

    train_shards = args.output_dir / "train_multiview_shards"
    valid_shards = args.output_dir / "valid_multiview_shards"
    train_output = args.output_dir / "multiview_train.parquet"
    valid_output = args.output_dir / "multiview_valid.parquet"
    if args.force:
    ...
    width = len(MULTIVIEW_CONTENT_VIEWS) * args.tokens_per_view
    joined = [
        join_content_and_multiview(
            content_train,
            train_shards,
            train_output,
