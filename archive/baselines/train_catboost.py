...
from soc_threat.feature_schema import (  # noqa: E402
    ...
    NUMERIC_FEATURES as TABULAR_NUMERIC_FEATURES,
)
...
def normalize_features(frame: pd.DataFrame) -> pd.DataFrame:
    for column in TABULAR_CATEGORICAL_FEATURES:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the archived b0.1 CatBoost baseline")
    parser.add_argument(
        "--train",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "artifacts" / "b0_1_catboost",
    )
...
def main() -> None:
    args = parse_args()
    features = TABULAR_CATEGORICAL_FEATURES + TABULAR_NUMERIC_FEATURES
    ...
    train_pool = Pool(
        ...
        cat_features=TABULAR_CATEGORICAL_FEATURES,
    ...
    metrics.update(
        {
            "model": "b0.1_catboost_structured_no_absolute_time_no_raw_ids",
            "features": features,
            "categorical_features": TABULAR_CATEGORICAL_FEATURES,
